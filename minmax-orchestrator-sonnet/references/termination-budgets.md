...
## Strategy-repeat and search budgets

- A failed cycle may receive at most one unchanged-category repair attempt by default.
- If the same verified failure recurs, require a changed strategy or exit; never increase the budget merely to hide stalling.
- `deliberative_search` requires its own finite branch/search budget in addition to global loop budgets.
- Local microplanning cannot silently reset global iteration, retry, no-progress, time, or cost budgets.
- Replanning does not reset consumed budget unless a newly approved contract explicitly changes the hard budget.
- Same failure + same strategy is not progress.
