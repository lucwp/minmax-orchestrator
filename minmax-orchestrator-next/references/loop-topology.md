...
## Planning regime comes first

Planning regime and execution topology are separate axes. Planning regime defines how the plan evolves as information arrives; topology defines how execution responsibility is organized. In Loop Mode, select the planning regime from `planning-regimes.md` before choosing the topology below.

Examples of valid combinations include `hierarchical_adaptive + single_adaptive_loop`, `hierarchical_adaptive + orchestrator_workers`, `solver_assisted + deterministic_prompt_chain`, `reactive_stepwise + single_adaptive_loop`, and `deliberative_search + orchestrator_workers`.

Do not treat topology selection as a substitute for feasibility, observability, or planning-regime selection.
