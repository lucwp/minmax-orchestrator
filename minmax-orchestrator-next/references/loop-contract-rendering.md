...
Show the selected planning regime and local adaptation policy when they materially change how the user should understand execution (for example partial/dynamic observability, search, or solver-assisted planning). Do not dump PlanSpec fields that do not affect approval or mental model.
