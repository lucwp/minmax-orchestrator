...
3. Select the minimum sufficient planning regime before expensive substantive execution; freeze a healthy macroplan, adapt locally from new evidence, and replan globally only on an observable invalidation trigger.
...
19. In Loop Mode, choose an explicit primary planning regime and then an explicit primary topology class before compiling the contract; when added planning or coordination complexity has uncertain gain, choose the simpler mechanism.
...
30. **Planning regime and execution topology are orthogonal.** Planning regime defines how the plan evolves as information arrives; topology defines how execution responsibility is organized.
...
When Loop Mode activates, it overrides the normal immediate-execution lifecycle, but **does not** force delegation. Follow `references/loop-mode.md`. Classify feasibility/affordances, select the minimum sufficient planning regime, then select the minimum sufficient topology before planning implementation detail.
...
2. identify objective, terminal state, hard constraints, observability, feasibility/affordances, required context/actions, planning regime, completion evidence, and approval/side-effect boundary;
...
2. create one compact Task Model, run the Feasibility & Affordance Gate, choose a planning regime, and compile the smallest useful dependency graph/PlanSpec;
