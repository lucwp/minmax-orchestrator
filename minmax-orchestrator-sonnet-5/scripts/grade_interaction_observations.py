...
def score(results):
    ...
    return round(100*sum(1 for _,e in results if not e)/len(results))
def main():
    p=argparse.ArgumentParser(); p.add_argument('spec',type=Path); p.add_argument('observations',type=Path); p.add_argument('--allow-partial',action='store_true'); a=p.parse_args(); s=json.loads(a.spec.read_text(encoding='utf-8')); o=json.loads(a.observations.read_text(encoding='utf-8')); verr=v.validate(s)
    if verr:
    ...
    sc=score(r); print(f'SCORE: {sc}/100'); return 0 if sc==s['scoring']['pass_score'] and all(not e for _,e in r) else 1
